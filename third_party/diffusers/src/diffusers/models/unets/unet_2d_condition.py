...
class UNet2DConditionModel(
    ...
    def _set_pos_net_if_use_instdiff_bbox_only(self, cross_attention_dim: int):
        """Set position_net to bbox-only version for ReferenceNet models."""
        from ..embeddings import INSTDIFFTextBoundingboxProjectionBBoxOnly
        ...
        self.position_net = INSTDIFFTextBoundingboxProjectionBBoxOnly(
            positive_len=positive_len,
            out_dim=out_dim,
        )
    ...
    def forward(
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if cross_attention_kwargs is not None and cross_attention_kwargs.get("instdiff", None) is not None:
                # NOTE(InstanceDiffusion/Stage2):
                # `cross_attention_kwargs["instdiff"]` is used for grounding (position_net + post-attn fuser) and
                # *optionally* ScaleU. However, vanilla SD inpaint UNets can be augmented with `position_net`/`fuser`
                # without being instantiated with `attention_type="unifusion"` (so ScaleU params are absent).
                # In that case, we must skip ScaleU to avoid AttributeError like missing `scaleu_b_0`.
                attention_type = getattr(getattr(self, "config", None), "attention_type", None)
                has_scaleu = hasattr(self, "scaleu_b_0") and hasattr(self, "scaleu_s_0")
                if attention_type == "unifusion" and has_scaleu:
                    scaleu_kwargs = {
                        j: {

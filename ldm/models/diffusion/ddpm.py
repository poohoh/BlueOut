...
try:
    ...
    try:
        ...
        try:
            from pytorch_lightning.utilities.distributed import rank_zero_only  # old PL
        except Exception:
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        _logvar = torch.full(fill_value=logvar_init, size=(self.num_timesteps,))
        if self.learn_logvar:
            self.logvar = nn.Parameter(_logvar, requires_grad=True)
        else:
            # keep on device with module for safe indexing with device tensor t
            self.register_buffer('logvar', _logvar, persistent=False)
    ...
    def training_step(self, batch, batch_idx):
        # 표준화된 batch_size를 로그에 전달해 PL의 모호성 경고를 방지
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log_dict(loss_dict_no_ema, prog_bar=False, logger=True, on_step=False, on_epoch=True, batch_size=batch_size)
        self.log_dict(loss_dict_ema, prog_bar=False, logger=True, on_step=False, on_epoch=True, batch_size=batch_size)
    ...
    def log_images(self, batch, N=8, n_row=2, sample=True, return_keys=None, **kwargs):
        ...
        x = x.to(self.device, non_blocking=True)[:N]
        log["inputs"] = x
...
class LatentDiffusion(DDPM):
    ...
    def make_cond_schedule(self, ):
        ...
        # keep cond_ids on the same device as the module for safe indexing
        if isinstance(getattr(self, '_buffers', None), dict) and 'cond_ids' in self._buffers:
            # update existing buffer in-place while keeping it registered
            self.cond_ids.data.copy_(cond_ids)
        else:
...
class LatentFinetuneDiffusion(LatentDiffusion):
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        ...
        missing, unexpected = self.load_state_dict(sd, strict=False) if not only_model else self.model.load_state_dict(
            sd, strict=False)
        # Filter out global U-Net-related keys from detailed lists only
        def _filter_keys(keys):
            # Suppress global U-Net-related entries and auxiliary fusion/tokenizer modules
            suppressed_prefixes = (
                'global_model',
                'patch_unifusion',
                'bank_gating_fusers',
            )
            return [k for k in keys if not any(str(k).startswith(p) for p in suppressed_prefixes)]

        mk = _filter_keys(missing)
        uk = _filter_keys(unexpected)
        print(f"Restored from {path} with {len(mk)} missing and {len(uk)} unexpected keys")
        if mk:
            print(f"Missing Keys: {mk}")
        if uk:
            print(f"Unexpected Keys: {uk}")

...
from InstanceDiffusion.ldm.modules.attention import SpatialTransformer
...
def Fourier_filter(x_in, threshold, scale):
    """Apply a simple frequency-domain filter.

    Always compute FFT in float32 with autocast disabled to avoid ComplexHalf
    issues under AMP. Cast back to input dtype on return.
    """
    B, C, H, W = x_in.shape

    # Run this block in full precision regardless of outer autocast context
    with torch.autocast(device_type="cuda", enabled=False):
        x32 = x_in.to(dtype=torch.float32)

        # FFT in float32 (avoids ComplexHalf warnings/instability)
        x_freq = fft.fftn(x32, dim=(-2, -1))
        x_freq = fft.fftshift(x_freq, dim=(-2, -1))

        # Build mask (float32) and apply scale in the center window
        _, _, Hf, Wf = x_freq.shape
        mask = torch.ones((B, C, Hf, Wf), device=x32.device, dtype=torch.float32)
        crow, ccol = Hf // 2, Wf // 2
        # ensure scale is float32 on the same device (grad not needed for mask)
        scale32 = scale.to(device=x32.device, dtype=torch.float32)
        mask[..., crow - threshold : crow + threshold, ccol - threshold : ccol + threshold] = scale32
        x_freq = x_freq * mask

        # IFFT back to spatial domain
        x_freq = fft.ifftshift(x_freq, dim=(-2, -1))
        x_filtered32 = fft.ifftn(x_freq, dim=(-2, -1)).real

    return x_filtered32.to(dtype=x_in.dtype)
...
class ResBlock(TimestepBlock):
    ...
    def forward(self, x, emb):
        ...
        if self.use_checkpoint and x.requires_grad:
            # Use non-reentrant checkpoint to avoid DDP "mark variable ready twice"
            # when the same parameters participate across multiple autograd edges.
...
class UNetModel(nn.Module):
    def __init__(
        ...
        # Disable training-time grounding dropout by default. Some upstream
        # repos randomly drop all grounding inputs (10%) to mimic CFG, but in
        # our pipeline does not perform CFG. Keep this at 0.0 to
        # avoid weakening reference supervision.
        self.grounding_dropout_p = 0.0
    ...
    def forward_single_input(self, input):
        ...
        #     device = grounding_input['boxes'].device
        #     grounding_input = self.grounding_tokenizer_input.get_null_input(batch=batch_size, device=device)

        # Optional random drop for guidance (disabled by default).
        # Set self.grounding_dropout_p>0 to re‑enable.
        if self.training and getattr(self, 'grounding_dropout_p', 0.0) > 0.0 and self.grounding_tokenizer_input.set:
            import random as _rand
            p = float(getattr(self, 'grounding_dropout_p', 0.0))
            if _rand.random() < p:
                batch_size = grounding_input['boxes'].size(0)
                device = grounding_input['boxes'].device
                grounding_input = self.grounding_tokenizer_input.get_null_input(batch=batch_size, device=device)
        ...
        # Original (full args):
        # objs, drop_box_mask = self.position_net(
        #     grounding_input['boxes'], grounding_input['masks'], grounding_input['positive_embeddings'],
        #     grounding_input['scribbles'], grounding_input['polygons'], grounding_input['segs'], grounding_input['points']
        # )
        # BBOX-ONLY call: pass only boxes, masks, positive_embeddings
        objs, drop_box_mask = self.position_net(
            grounding_input['boxes'],
            grounding_input['masks'],
            grounding_input['positive_embeddings'],
        )
        ...
        # Pass grounding_input when att_masks or visual_valid_mask is present.
        use_grounding = isinstance(grounding_input, dict) and (
            ('att_masks' in grounding_input) or ('visual_valid_mask' in grounding_input)
        )

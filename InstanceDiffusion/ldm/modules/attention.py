...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, key, value, mask=None):
        ...
        if self.efficient_attention:
            # Allow PyTorch to choose an appropriate kernel and provide a safe fallback.
            try:
                ...
                # Final fallback to math kernel which is always available.
                with torch.backends.cuda.sdp_kernel(
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, grounding_input=None, drop_box_mask=False): 
        ...
        # Extract visual_valid_mask regardless of efficient_attention.
        if isinstance(grounding_input, dict) and "visual_valid_mask" in grounding_input:
            visual_valid_mask = grounding_input["visual_valid_mask"]

        # flash attention does not support the attention masking yet
        # Keep att_masks handling disabled when efficient_attention is enabled.
        if grounding_input is not None and not self.efficient_attention:
            if isinstance(grounding_input, dict) and "att_masks" in grounding_input:
                ...
                if att_masks_reshape is not None and torch.sum(att_masks_reshape) > 0.0 and not drop_box_mask:
                    ...
                    if att_key in grounding_input:
                        ...
                        # 각 객체마다 각 위치간에 동일한 객체에 속하는지를 계산한 마스크
                        ...
                        # 모든 객체에 대해서 각 위치들 간에 동일한 객체에 속하는 경우가 있는지에 대한 마스크
        ...
        # Apply optional letterbox-valid visual mask to self-attention (mask out padding tokens)
        # Only mask keys: prevent all queries from attending to invalid (padding) keys.
        # Note: The visual_valid_mask is defined at the latent resolution (e.g. 64x64),
        # so we only apply it when the number of tokens N is at least that many.
        if visual_valid_mask is not None:
            vv = visual_valid_mask  # visual valid
            if vv.dim() == 4 and vv.shape[1] != 1:
                vv = vv[:, :1]
            B_v = vv.shape[0]  # batch
            h_lat, w_lat = vv.shape[-2], vv.shape[-1]  # latent height/width
            w_h_v = h_lat * w_lat  # num latent tokens
            vv = (vv > 0.5).float()
            vv_key = vv.view(B_v, 1, 1, w_h_v)  # (B,1,1,w_h_v) key mask

            # Ensure att_masks_ exists and has the correct base shape.
            if att_masks_ is None:
                att_masks_ = torch.ones(B, 1, N, N, dtype=vv.dtype, device=vv.device)

            # Guard against cases where the current attention layer has fewer tokens
            # than the latent mask (e.g., lower-resolution stages). In that case,
            # skip applying the visual_valid_mask to avoid shape mismatches.
            if N >= w_h_v:
                att_masks_[:, :, :w_h_v, :w_h_v] = (
                    att_masks_[:, :, :w_h_v, :w_h_v] * vv_key
                )

        # Expose the final attention mask for debugging/visualization hooks
        self._debug_att_masks = att_masks_

        if self.efficient_attention:
            # Convert {0,1} multiplicative mask to additive mask for SDPA
            # 0 -> -inf (masked), 1 -> 0 (not masked)
            att_masks_sdpa = None
            if att_masks_ is not None:
                att_masks_sdpa = torch.where(
                    att_masks_ <= 0.0,
                    torch.tensor(-float('inf'), device=att_masks_.device, dtype=att_masks_.dtype),
                    torch.tensor(0.0, device=att_masks_.device, dtype=att_masks_.dtype)
                )
            # Allow PyTorch to choose an appropriate kernel and provide a safe fallback.
            try:
                with torch.backends.cuda.sdp_kernel(
                    ...
                    out = F.scaled_dot_product_attention(q, k, v, attn_mask=att_masks_sdpa)
            except RuntimeError:
                # Final fallback to math kernel which is always available.
                with torch.backends.cuda.sdp_kernel(
                    enable_flash=False, enable_math=True, enable_mem_efficient=False
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, context, objs, grounding_input=None, drop_box_mask=False):
        if self.use_checkpoint and x.requires_grad:
            # Use non-reentrant checkpoint so grads can flow to tensors
            # captured by closures (e.g., reference tokens).
            return checkpoint.checkpoint(self._forward, x, context, objs, grounding_input, use_reentrant=False)
        else:
